<template>
  <div class="bg-black text-white min-vh-100">
    ...
    <div
      ...
      <div class="logo fw-bold fs-3">TECH<span class="text-danger">STORE</span></div>
...
const confirmarPedido = async () => {
  ...
  if (!authStore.isAuthenticated && !guestMode.value) {
    // Mostrar modal y esperar la decisión
    return new Promise<void>((resolve) => {//Promise para esperar la respuesta del modal
      const handler = () => {//Función para manejar la respuesta del modal
        guestMode.value = true
        window.removeEventListener('checkout:guest', handler)//Eliminar el evento
        resolve()
      }
      window.addEventListener('checkout:guest', handler, { once: true })//esta parte es para que el checkout sepa que puede continuar
      authModalRef.value?.show()//Mostrar el modal
    }).then(() => {
      // Una vez elegido invitado, volver a llamar a la función (sin volver a preguntar)
      confirmarPedido()
    })
  }
