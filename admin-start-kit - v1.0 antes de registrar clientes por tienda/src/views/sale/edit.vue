        ...
        <div class="d-flex flex-wrap justify-space-between gap-4 mb-6">
            <div class="d-flex flex-column justify-center">
                <h4 class="text-h4 mb-1">
                    💰 Editar Sale/Cotización : {{ sale_id_edit }}
                </h4>
                            ...
                            <table class="table datatable">
                                ...
                                <tbody>
                                    <tr v-for="(sale_detail, index) in sale_details" :key="index">
                                        ...
                                        <td><b-form-input type="number" v-model="sale_detail.quantity" /></td>
                                        <td><b-form-input v-model="sale_detail.price_base" /></td>
...
import type { SaleConfig, Sale, SaleDetail, SalePayment, SaleResponse, SaleDetailResponse } from '@/types/sales';
...
const discount_total = ref<number>(0);//descuento por item
const discount_global = ref<number>(0);//descuento global
...
const route = useRoute("sale.edit");//esto es para redireccionar en la url y poder obtener el id
const sale_id_edit = ref<string | string[]>("");//id de la venta a edit
const sale_selected = ref<Sale | undefined>(undefined);    //venta seleccionada para editar
...
const addProduct = async () => {
    ...
    if (!sale_selected.value) {
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'No hay venta seleccionada',
        });
        return;
    }
    ...
    const existingDetail = sale_details.value.find(
        (d) => d.product.id === product_selected.value.id
    );
    ...
    try {
        ...
        (Swal as TVueSwalInstance)
            .fire({
                ...
                text: `¿Estás seguro de eliminar este producto '${SALE_DETAIL.product.title}'?`,
                icon: "warning",
            ...
            .then(async (result: any) => {
                if (result.isConfirmed) {
                    const res: AxiosResponse<SaleDetailResponse> = await httpClient.delete("sale_details/" + SALE_DETAIL.id); //borrar el detalle
                    const res2: AxiosResponse<any> = await httpClient.delete("sale_payments/" + SALE_PAYMENT.id);//borrar el pago
                    (Swal as TVueSwalInstance).fire(
                        "Eliminado!",
                        `El producto [${SALE_DETAIL.product.title}] ha sido eliminado con éxito.`,
                        "success"
                    );
...
const sumDetails = () => {
    ...
    const details = sale_details.value ?? [];
    const payments = sale_payments.value ?? [];

    igv_total.value = Number((details
        .filter((d: any) => ![11].includes(Number(d.tip_afe_igv)))
        .reduce((sum: number, d: any) => sum + Number(d.igv || 0), 0)
    ).toFixed(4));

    icbper_total.value = Number((details
        .filter((d: any) => ![11].includes(Number(d.tip_afe_igv)))
        .reduce((sum: number, d: any) => sum + Number(d.icbper || 0), 0)
    ).toFixed(4));

    isc_total.value = Number((details
        .filter((d: any) => ![11].includes(Number(d.tip_afe_igv)))
        .reduce((sum: number, d: any) => sum + Number(d.isc || 0), 0)
    ).toFixed(4));

    sale_subtotal.value = Number((details
        .filter((d: any) => ![11].includes(Number(d.tip_afe_igv)))
        .reduce((sum: number, d: any) => sum + Number(d.subtotal || 0), 0)
    ).toFixed(4));

    discount_total.value = Number((details
        .filter((d: any) => ![11].includes(Number(d.tip_afe_igv)))
        .reduce((sum: number, d: any) => sum + Number(d.discount || 0), 0)
    ).toFixed(4));

    total_payments.value = Number((payments
...
const removePayment = (index: number) => {
    ...
    try {
        (Swal as TVueSwalInstance)
            .fire({
                title: "Confirmar la eliminación",
                text: `¿Estás seguro de eliminar este pago '${SALE_PAYMENT.method_payment}''?`,
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: "#3085d6",
                cancelButtonColor: "#d33",
            ...
            .then(async (result: any) => {
                if (result.isConfirmed) {
                    ...
                    (Swal as TVueSwalInstance).fire(
                        "Eliminado!",
                        `El metodo de pago [${SALE_PAYMENT.method_payment}] ha sido eliminado con éxito.`,
                        "success"
                    );
...
const store = async () => {
    try {
        ...
        const resp: AxiosResponse<SaleResponse> = await httpClient.patch("sales/" + sale_id_edit.value, data);
        //console.log(resp);
        if (resp.data.error) {
            (Swal as TVueSwalInstance).fire(
...
const show = async () => {
    try {
        const res: AxiosResponse<SaleResponse> = await httpClient.get(`sales/${sale_id_edit.value}`);
        console.log(res);
        ...
        if (sale_selected.value) {
            ...
            sale_payments.value = sale_selected.value.sale_payments;
            ...
            //  total_retencion.value = sale_selected.value.total_retencion;
            //  total_detraccion.value = sale_selected.value.total_detraccion;
            //  total_percepcion.value = sale_selected.value.total_percepcion;
             discount_global.value = Number(sale_selected.value.discount_global);
            setTimeout(() => {
                sumDetails();
            }, 25);
        }
...
onMounted(() => {
    sale_id_edit.value = route.params.id;
    config();
});
</script>