<template>
    <DefaultLayout>
        <b-row class="justify-content-center">
            <b-col cols="12">
                <b-card-header>
                    <b-card-title>Conceptos de Caja</b-card-title>
                ...
                <b-card-body class="pt-0 mt-2">
                    <b-table-simple responsive class="mb-0 table-centered">
                        ...
                        <b-tbody>
                            <b-tr v-for="(item, index) in cashConcepts" :key="index">
                                <b-td>{{ cashConcepts.length - index }}</b-td>
                                <b-td>{{ item.name }}</b-td>
                                <b-td>
                                    <b-badge variant="success" v-if="item.direction === 'in'">Ingreso</b-badge>
                                    <b-badge variant="warning" v-else>Egreso</b-badge>
                                </b-td>
                                <b-td>
        ...
        <b-modal v-model="ModalRegister" :title="`${itemSelected ? 'Edición' : 'Registro'} de Concepto de Caja`"
            :header-class="`bg-${themeColor}`" title-class="m-0 text-white" :ok-variant="themeColor" hide-footer
            centered size="lg">
            <b-row>
                <b-col lg="6">
                    <label for="name-cash-concept" class="col-form-label text-lg-end">Nombre: </label>
                    <b-form-input type="text" id="name-cash-concept" placeholder="Example: Pago a proveedor"
                        v-model="name" />
                </b-col>
                <b-col lg="6">
                    <label for="direction-cash-concept" class="col-form-label text-lg-end">Dirección: </label>
                    <b-form-select id="direction-cash-concept" v-model="direction">
...
import type { CashConcept, CashConcepts, CashConceptResponse } from '@/types/cash';
...
const cashConcepts = ref<CashConcept[]>([]);

const name = ref<string>("");
const direction = ref<'in' | 'out'>('out');
const is_active = ref<boolean>(true);

const itemSelected = ref<CashConcept | undefined>(undefined);

const list = async () => {
    try {
        const res: AxiosResponse<CashConcepts> = await httpClient.get(
            `cash-concepts?page=${currentPage.value}&search=${search.value ?? ''}`);

        cashConcepts.value = res.data.cash_concepts;
...
const store = async () => {
    try {
        if (name.value.trim() === '') {
            ...
            await swal.fire('Error', 'El nombre del concepto es obligatorio.', 'error');
            return;
        }
        ...
        const res: AxiosResponse<CashConceptResponse> =
            !itemSelected.value
                ? await httpClient.post("cash-concepts", payload)
                : await httpClient.put("cash-concepts/" + itemSelected.value?.id, payload);

        ModalRegister.value = false;

        if (!itemSelected.value) {
            if (res.data.cash_concept) {
                cashConcepts.value.unshift(res.data.cash_concept);
            }
        } else {
            const INDEX = cashConcepts.value.findIndex((it) => it.id == itemSelected.value?.id);
            if (INDEX != -1 && res.data.cash_concept) {
                cashConcepts.value[INDEX] = res.data.cash_concept;
            }
        }
...
const editItem = (item: CashConcept) => {
...
const toggleActive = (item: CashConcept) => {
    (Swal as TVueSwalInstance)
        .fire({
            title: item.is_active ? 'Confirmar desactivación' : 'Confirmar activación',
            text: item.is_active
                ? `¿Desactivar el concepto '${item.name}'?`
                : `¿Activar el concepto '${item.name}'?`,
            icon: "warning",
        ...
        .then(async (result: any) => {
            ...
            try {
                if (item.is_active) {
                    await httpClient.delete("cash-concepts/" + item.id);
                } else {
                    await httpClient.put("cash-concepts/" + item.id, {
                        name: item.name,
