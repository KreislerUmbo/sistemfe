...
// tax_params y detracciones se cargan en config() — una sola llamada HTTP
...
const getPriceBaseCF = (): string => {
  if (include_igv.value == 0) return price_general.value.toFixed(6);
  const divisor = 1 + (getTasaActual() / 100);  // 1.18, 1.04, o 1.00
  return (price_general.value / divisor).toFixed(6);
};

const getPriceBaseCE = (): string => {
  if (include_igv.value == 0) return price_company.value.toFixed(6);
  const divisor = 1 + (getTasaActual() / 100);
  return (price_company.value / divisor).toFixed(6);
};
...
const store = async () => {
  try {
    ...
    if (tipo_isc.value === '02') {
      ...
      formData.append('monto_isc_fijo', '0');
      formData.append('contenido_neto_litros', '');
    }
    ...
    if (FILE_IMAGEN.value) {
      const file = FILE_IMAGEN.value as File;
...
onMounted(() => {
  config(); // carga categorías + taxes + detracciones en una sola llamada
});
