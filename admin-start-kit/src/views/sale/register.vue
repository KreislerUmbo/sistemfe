                            ...
                            <table class="table datatable">
                                ...
                                <tbody>
                                    <tr v-for="(sale_detail, index) in sale_details" :key="index">
                                        ...
                                        <td v-if="![11].includes(sale_detail.tip_afe_igv)">{{ currency }} {{
                                            sale_detail.subtotal }}</td>
...
import type { SaleConfig, Sale, SaleDetail, SalePayment } from '@/types/sales';
...
const addProduct = () => {
    ...
    if (quantity.value <= 0) {
        (Swal as TVueSwalInstance).fire({
            icon: 'error',
            title: 'Error',
            text: 'Necesitas digitar la cantidad del producto',
        });
        return;
    }
    ...
    let IGV = SUBTOTAL * PERCENTAGE_IGV;
    let PRECIO_FINAL = Number(((SUBTOTAL + IGV) / quantity.value).toFixed(3));
    ...
    sale_details.value.unshift({
        ...
        igv: IGV,
        subtotal: SUBTOTAL,
        per_icbper: 0, //porcentaje de icbper
        icbper: 0,
...
const sumDetails = () => {
    igv_total.value = Number((sale_details.value.reduce((sum: number, sale_detail: any) => (sum + sale_detail.igv), 0)).toFixed(3));
    sale_subtotal.value = Number((sale_details.value.reduce((sum: Number, sale_detail: any) => sum + sale_detail.subtotal, 0)).toFixed(3));
    discount_total.value = Number((sale_details.value.reduce((sum: number, sale_detail) => sum + sale_detail.discount, 0)).toFixed(3));
    sale_total.value = Number((igv_total.value + sale_subtotal.value).toFixed(3));
}
const getIgvTotal = () => {
    return igv_total.value;
}
const getSubTotalSale = () => {
    return sale_subtotal.value;
}
const getTotalSales = () => {
    return sale_total.value;
}
const addPayment = () => {
...
const addProduct2 = () => {
    if (!price_unit.value || price_unit.value == 0) {
        // message_text.value = "Necesitas digitar el precio unitario del producto";
        return;
    }
    if (!quantity.value || quantity.value == 0) {
        // message_text.value = "Necesitas digitar la cantidad del producto";
        return;
    }
    if (!product_selected.value) {
        // message_text.value = "Necesitas seleccionar un producto";
        return;
    }

    let PRODUCT_EXITS = sale_details.value.find((sldt) => sldt.product.id == product_selected.value?.id);
    if (PRODUCT_EXITS) {
        message_text.value = "El producto ya se encuentra en la lista";
        // setTimeout(() => {
    ...
    let TOTAL_VENTA = (quantity.value * price_unit.value) - DISCOUNT_TOTAL;
    let IGV = TOTAL_VENTA * 0.18;
    let PRECIO_FINAL = Number(((TOTAL_VENTA + IGV) / quantity.value).toFixed(2));

    sale_details.value.push({
        product: product_selected.value,
        unidad_medida: product_selected.value.unidad_medida,
        price_base: price_unit.value,
        price_final: PRECIO_FINAL,
        quantity: quantity.value,
        discount: DISCOUNT_TOTAL,
