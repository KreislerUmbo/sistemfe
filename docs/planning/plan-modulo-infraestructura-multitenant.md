...
| 30-jul-2026 | **Segundo bug real de este mecanismo, distinto del de Sesión 2 — cerrado, rama `fix/infra-migracion-verticals-pendientes` (mergeada a `main` en `3fc2c6f`).** El de Sesión 2 era sobre `migrarVertical()` (provisioning inicial, cuando se crea un tenant). Este es sobre el camino de **mantenimiento** (agregar migraciones nuevas a un tenant que ya existe): `config/tenancy.php` `migration_parameters['--path']` está hardcodeado a `tenant/core/`, así que el comando genérico `php artisan tenants:migrate` (sin `--path`) nunca corría `tenant/verticals/*` para ningún tenant — venía pasando desde el primer vertical, compensado con migración manual cada sesión (confirmado real en Sesión 11b4b, `plan-hoja-de-ruta-ejecucion.md`). Corregido con un comando nuevo, `tenants:migrate-verticales`, que reemplaza a `tenants:migrate` a secas para este caso de uso de acá en adelante — corre `tenant/core/` para todos, agrupa por `giro`, corre `tenant/verticals/{giro}/` con `--path` explícito por grupo (reutiliza `TenantProvisioningService::rutaVertical()`, extraído de `migrarVertical()` para no duplicar el mapeo snake_case→kebab-case de la sección 1.1). Detalle completo, incluida la corrida real contra los tenants existentes, en `arquitectura-multitenant-backend_1.md` y `CLAUDE.md` (sección "Migración a multi-tenancy", pendientes conocidos). |
