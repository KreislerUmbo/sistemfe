...
| 11a | API REST + formularios maestros | Controllers/rutas para `proveedores`+tarifas, `destinos_atractivos`+`servicios`, `temporadas`, `guias`+tarifas, `configuracion_agencia`; pantallas Vue/Bootstrap 5 (`admin-start-kit`) para cada uno | `plan-modulo-cotizaciones-reservas.md` §2, §5.2, §5.3, §7 | [ ] |
